...
struct GammaCorrectionbatchPDLocalData { 
...
static vx_status VX_CALLBACK refreshGammaCorrectionbatchPD(vx_node node, const vx_reference *parameters, vx_uint32 num, GammaCorrectionbatchPDLocalData *data)
{
	...
	data->gamma = (vx_float32 *)malloc(sizeof(vx_float32) * arr_size);
	copy_status = vxCopyArrayRange((vx_array)parameters[4], 0, arr_size, sizeof(vx_float32),data->gamma, VX_READ_ONLY, VX_MEMORY_TYPE_HOST);
...
static vx_status VX_CALLBACK validateGammaCorrectionbatchPD(vx_node node, const vx_reference parameters[], vx_uint32 num, vx_meta_format metas[])
{
	vx_status status = VX_SUCCESS;
	vx_enum scalar_type;
	...
	{
		return ERRMSG(VX_ERROR_INVALID_FORMAT, "validate: GammaCorrectionbatchPD: image: #0 format=%4.4s (must be RGB2 or U008)\n", (char *)&df_image);
	}
...
static vx_status VX_CALLBACK uninitializeGammaCorrectionbatchPD(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
	GammaCorrectionbatchPDLocalData * data; 
	STATUS_ERROR_CHECK(vxQueryNode(node, VX_NODE_LOCAL_DATA_PTR, &data, sizeof(data)));
#if ENABLE_OPENCL
	if(data->device_type == AGO_TARGET_AFFINITY_GPU)
...
vx_status GammaCorrectionbatchPD_Register(vx_context context)
{
	vx_status status = VX_SUCCESS;
	// Add kernel to the context with callbacks
	vx_kernel kernel = vxAddUserKernel(context, "org.rpp.GammaCorrectionbatchPD",
		VX_KERNEL_RPP_GAMMACORRECTIONBATCHPD,
		processGammaCorrectionbatchPD,
		7,
		validateGammaCorrectionbatchPD,
		initializeGammaCorrectionbatchPD,
		uninitializeGammaCorrectionbatchPD);
