...
void ColorTemperatureNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _adj_value_param.set_batch_size(_inputs[0]->info().batch_size());
    _adj_value_param.create_array(graph ,VX_TYPE_INT32);

    _node = vxExtrppNode_ColorTemperaturebatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _adj_value_param.default_array(), _outputs[0]->info().batch_size());
