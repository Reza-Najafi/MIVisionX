...
void WarpAffineNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _affine.resize(6 * _outputs[0]->info().batch_size());    
    ...
    uint batch_size = _inputs[0]->info().batch_size();
    ...
    vx_status status;
    _affine_array = vxCreateArray(vxGetContext((vx_reference)_graph->get()), VX_TYPE_FLOAT32, _outputs[0]->info().batch_size() * 6);
    status = vxAddArrayItems(_affine_array,_outputs[0]->info().batch_size() * 6, _affine.data(), sizeof(vx_float32));
    _node = vxExtrppNode_WarpAffinebatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(),dst_width_array,dst_height_array, 
                                            _affine_array, _outputs[0]->info().batch_size());
...
void WarpAffineNode::update_affine_array()
{
    ...
    affine_status = vxCopyArrayRange((vx_array)_affine_array, 0, _outputs[0]->info().batch_size() * 6, sizeof(vx_float32), _affine.data(), VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST); //vxAddArrayItems(_width_array,_outputs[0]->info().batch_size(), _width, sizeof(vx_uint32));
    if(affine_status != 0)
