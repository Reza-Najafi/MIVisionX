...
void SnPNoiseNode::create(std::shared_ptr<Graph> graph)
{
    ...
    if(width_status != 0 || height_status != 0)
     THROW(" vxAddArrayItems failed in the snp noise (vxExtrppNode_NoisebatchPD) node: "+ TOSTR(width_status) + "  "+ TOSTR(height_status))

    _sdev.set_batch_size(_inputs[0]->info().batch_size());
    _sdev.create_array(graph ,VX_TYPE_FLOAT32);
    _node = vxExtrppNode_NoisebatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _sdev.default_array(), _outputs[0]->info().batch_size());
...
void SnPNoiseNode::update_dimensions()
{
    ...
    if(width_status != 0 || height_status != 0)
        THROW(" vxCopyArrayRange failed in the snp noise (vxExtrppNode_NoisebatchPD) node: "+ TOSTR(width_status) + "  "+ TOSTR(height_status))
