...
void SatNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _node = vxExtrppNode_SaturationbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _sat.default_array(), _outputs[0]->info().batch_size());
    vx_status status;
