...
void ExposureNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _node = vxExtrppNode_ExposurebatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _shift.default_array(), _outputs[0]->info().batch_size());
