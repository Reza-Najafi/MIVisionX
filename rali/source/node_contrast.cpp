...
void RaliContrastNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _min.set_batch_size(_inputs[0]->info().batch_size());
    _min.create_array(graph ,VX_TYPE_UINT32);
    _max.set_batch_size(_inputs[0]->info().batch_size());
    _max.create_array(graph ,VX_TYPE_UINT32);
    _node = vxExtrppNode_ContrastbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _min.default_array(), _max.default_array(), _outputs[0]->info().batch_size());
