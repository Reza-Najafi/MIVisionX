...
{
    vx_status width_status, height_status, gamma_status;
    if(_node)
        return;
    ...
    _shift.set_batch_size(_inputs[0]->info().batch_size());
    _shift.create_array(graph ,VX_TYPE_FLOAT32);
    _node = vxExtrppNode_GammaCorrectionbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _shift.default_array(), _outputs[0]->info().batch_size());
