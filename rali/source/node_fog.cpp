...
void FogNode::create(std::shared_ptr<Graph> graph)
{
    ...
    vx_float32 fog = _fog_param.default_value(); //_sdev.default_value();
    vx_scalar fog_scalar = vxCreateScalar(vxGetContext((vx_reference)_graph->get()), VX_TYPE_FLOAT32, &fog);
    ...
    _fog_param.set_batch_size(_inputs[0]->info().batch_size());
    _fog_param.create_array(graph ,VX_TYPE_FLOAT32);
    _node = vxExtrppNode_FogbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _fog_param.default_array(), _outputs[0]->info().batch_size());
