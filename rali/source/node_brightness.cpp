...
void BrightnessNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _node = vxExtrppNode_BrightnessbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _alpha.default_array(), _beta.default_array(), _outputs[0]->info().batch_size());/*A temporary fix for time being*/
