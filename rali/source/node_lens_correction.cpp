...
void LensCorrectionNode::create(std::shared_ptr<Graph> graph)
{
    ...
    vx_float32 strength = _strength.default_value(); //_sdev.default_value();
    vx_scalar strength_scalar = vxCreateScalar(vxGetContext((vx_reference)_graph->get()), VX_TYPE_FLOAT32, &strength);
    vx_float32 zoom = _zoom.default_value(); //_sdev.default_value();
    vx_scalar zoom_scalar = vxCreateScalar(vxGetContext((vx_reference)_graph->get()), VX_TYPE_FLOAT32, &zoom);
    ...
    _strength.set_batch_size(_inputs[0]->info().batch_size());
    _strength.create_array(graph ,VX_TYPE_FLOAT32);
    _zoom.set_batch_size(_inputs[0]->info().batch_size());
    _zoom.create_array(graph ,VX_TYPE_FLOAT32);
    _node = vxExtrppNode_LensCorrectionbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _strength.default_array(), _zoom.default_array(), _outputs[0]->info().batch_size());
