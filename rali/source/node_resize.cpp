...
void ResizeNode::create(std::shared_ptr<Graph> graph)
{
    ...
    height_status = vxAddArrayItems(dst_height_array,_outputs[0]->info().batch_size(), dst_height.data(), sizeof(vx_uint32));
     if(width_status != 0 || height_status != 0)
        THROW(" vxAddArrayItems failed in the resize (vxExtrppNode_ResizebatchPD) node: "+ TOSTR(width_status) + "  "+ TOSTR(height_status))
   _node = vxExtrppNode_ResizebatchPD(_graph->get(), _inputs[0]->handle(),src_width_array,src_height_array, _outputs[0]->handle(),dst_width_array,dst_height_array,_inputs[0]->info().batch_size());
