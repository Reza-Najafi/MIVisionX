...
void RotateNode::create(std::shared_ptr<Graph> graph)
{
    ...
    if(width_status != 0 || height_status != 0)
        THROW(" vxAddArrayItems failed in the rotate (vxExtrppNode_RotatebatchPD) node: "+ TOSTR(width_status) + "  "+ TOSTR(height_status))
    _angle.set_batch_size(_inputs[0]->info().batch_size());
    _angle.create_array(graph ,VX_TYPE_FLOAT32);
   _node = vxExtrppNode_RotatebatchPD(_graph->get(), _inputs[0]->handle(),_width_array, _height_array, _outputs[0]->handle(),dst_width_array,dst_height_array, _angle.default_array(), _outputs[0]->info().batch_size());
