...
void FlipNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _flip_axis.set_batch_size(_inputs[0]->info().batch_size());
    _flip_axis.create_array(graph ,VX_TYPE_UINT32);
    _node = vxExtrppNode_FlipbatchPD(_graph->get(), _inputs[0]->handle(), _width_array, _height_array, _outputs[0]->handle(), _flip_axis.default_array(), _outputs[0]->info().batch_size());
